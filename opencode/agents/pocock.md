...
3. Scan for signature files: `src/worker.ts`, `*-do.ts`.
...
- Announce what you detected and what you loaded, briefly, so the user can see the reasoning. Example: *"Detected `wrangler.jsonc` and a Durable Object class in this project — loading `cloudflare`, `workers-best-practices`, and `durable-objects` before entering Phase 5."*
